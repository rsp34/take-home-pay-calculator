...
std::shared_ptr<TaxRegistry> createUKTaxRegistry(std::shared_ptr<UKTaxCode> taxCode){
    ...
    registry->registrations_[UKTaxNames::INCOME_TAX] = TaxRegistration {std::make_shared<UKIncomeTax>(taxCode), std::make_shared<CumulativeTaxStrategy>()};
    registry->registrations_[UKTaxNames::EMPLOYEE_NI] = TaxRegistration {std::make_shared<UKEmployeeNI>(taxCode), std::make_shared<CumulativeTaxStrategy>()};
    return registry;
}